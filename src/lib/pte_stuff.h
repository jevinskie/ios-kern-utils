...
void checkvad() {
    if (!sz) {
        ...
        fprintf(stderr, "checkvad: %lx %x", sz, getpagesize());
...
void parse_block_tte(uint64_t tte) {
    fprintf(stderr, "TTE physaddr: %016llx", TTE_GET(tte, TTE_PHYS_VALUE_MASK));
    fprintf(stderr, "TTE table: %d", !!(TTE_GET(tte, TTE_IS_TABLE_MASK)));
    fprintf(stderr, "TTE valid: %d", !!(TTE_GET(tte, TTE_IS_VALID_MASK)));
    fprintf(stderr, "TTE userland eXecute Never: %d", !!(TTE_GET(tte, TTE_BLOCK_ATTR_UXN_MASK)));
    fprintf(stderr, "TTE privileged eXecute Never: %d", !!(TTE_GET(tte, TTE_BLOCK_ATTR_PXN_MASK)));
    fprintf(stderr, "TTE contiguous: %d", !!(TTE_GET(tte, TTE_BLOCK_ATTR_CONTIG_MASK)));
    fprintf(stderr, "TTE not global: %d", !!(TTE_GET(tte, TTE_BLOCK_ATTR_NG_MASK)));
    fprintf(stderr, "TTE access flag: %d", !!(TTE_GET(tte, TTE_BLOCK_ATTR_AF_MASK)));
    fprintf(stderr, "TTE shareable: %01llx", (TTE_GET(tte, TTE_BLOCK_ATTR_SH_MASK) >> 8));
    fprintf(stderr, "TTE access permissions: %s", (TTE_GET(tte, TTE_BLOCK_ATTR_AP_RO_MASK)) ? "Read only" : "Read/Write");
    fprintf(stderr, "TTE access permissions: %s", (TTE_GET(tte, TTE_BLOCK_ATTR_AP_CAN_EL0_ACCESS_MASK)) ? "EL0 (Userland) can access this page" : "EL0 (Userland) cannot access this page");
}
